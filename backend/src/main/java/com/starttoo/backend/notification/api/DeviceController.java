...
@Tag(name = "Devices", description = "Firebase Installation ID와 로그인 기기 관리")
...
public class DeviceController {
    ...
    @Operation(
            ...
            description = """
                    fid를 전역 고유키로 upsert한다. 신규 FID이면 기기 행을 생성하고 기존 FID이면
                    현재 회원·플랫폼으로 연결을 갱신하면서 활성화하고 마지막 사용 시각을
                    기록한다. 요청의 현재 리프레시 토큰을 해당 deviceSeq에 연결하므로 기기
                    비활성화 시 그 기기의 세션만 폐기할 수 있다. 현재 리프레시 토큰에 다른
                    deviceSeq가 연결되어 있으면 같은 트랜잭션에서 이전 기기를 비활성화하고 새
                    FID 기기에 연결한다. 리프레시 토큰의 회원과 JWT 회원이 다르면 거부한다.
                    """
    )
