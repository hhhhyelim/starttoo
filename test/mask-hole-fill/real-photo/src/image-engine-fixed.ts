...
/**
 * [수정] 마스크 바깥 테두리와 연결되지 않은 저신뢰 영역(=고신뢰 인물 픽셀에
 * 완전히 둘러싸인 섬)을 인물로 승격한다. removeTattooBackground()가 도안
 * 배경을 지울 때 쓰는 테두리 연결 flood-fill과 동일한 원리를 인물 마스크에
 * 적용해, 흉터·문신·반사광처럼 국소적으로 오탐된 구멍만 골라 메운다.
 * 다리 사이 틈처럼 테두리와 실제로 이어진 배경은 그대로 유지된다.
 */
