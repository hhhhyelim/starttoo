...
from .embed import line_probes, topk_multiprobe
...
class Searcher:
    ...
    def search(self, mask_png: bytes, mode: str = "line", top_k: int = 16,
        ...
        mode 는 와이어 호환을 위해 남겨 둔 인자다. 면(gate) 모드를 걷어냈으므로
        line 만 받는다.
        """
        if mode != "line":
            raise BadRequest(f"mode 는 line 만 지원한다: {mode}")
        ...
        rows, staged = self._candidates(probes, candidates)
