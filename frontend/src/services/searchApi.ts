...
/**
 * GET /search/accounts/autocomplete — 회원 닉네임 자동완성 (1글자부터)
 *
 * 본 검색(/search/accounts)은 두 글자 이상만 받는다. 한 글자일 때는 이쪽을 쓴다.
 */
export async function autocompleteAccounts(
	q: string,
	size = 10,
): Promise<AccountResult[]> {
	const { data } = await api.get<AccountResult[]>(
...
/** GET /search/accounts — 회원 닉네임 검색 (두 글자 이상) */
...
/**
 * GET /search/artists/autocomplete — 인증 아티스트 닉네임 자동완성 (1글자부터)
 *
...
export async function autocompleteArtists(
	...
	const { data } = await api.get<AccountResult[]>(
		"/search/artists/autocomplete",
		{ params: { q, size } },
	);
	return data;
}

/** GET /search/artists — 인증 아티스트 닉네임 검색 (두 글자 이상) */
