...
export const BRUSH_PX: Record<SearchMode, number> = {
	coverup: 16,
	// 엔진 튜닝 기준값은 6px 이지만, 그 굵기로는 화면에서 선이 너무 얇아 가릴 부위를
	// 따라 그리기가 불편하다는 의견이 있어 슬라이더(4~20)의 40% 지점인 10px 로 올렸다.
	// 사용성을 위해 기준값에서 의도적으로 벗어난 값이다 — 위 주석대로 검색 품질이
	// 조용히 나빠질 수 있으므로, 추천 결과가 예전만 못하다는 이야기가 나오면 여기부터
	// 6px 로 되돌려 보고 판단할 것.
	shape: 10,
};
