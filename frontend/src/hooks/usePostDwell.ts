...
/**
 * 이 방문에서 이미 체류시간을 보낸 게시물.
 *
 * 서버는 원본 체류시간이나 사용자×게시물 행을 저장하지 않고 받은 초를 즉시 취향
 * 점수에 더하기만 한다. 중복 방지가 서버에 없어서 같은 게시물을 여닫기만 반복해도
 * 점수가 무한히 쌓이므로 게시물당 한 번으로 막는다.
 */
const sentPostIds = new Set<number>();

/**
 * 게시물 상세를 보고 있는 시간을 재서 취향 점수에 반영한다.
 *
